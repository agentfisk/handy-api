...
module HandyAPI
  class GetRoundTripDelay200ResponseOneOf
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `HandyAPI::GetRoundTripDelay200ResponseOneOf` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `HandyAPI::GetRoundTripDelay200ResponseOneOf`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'rtd')
        self.rtd = attributes[:'rtd']
      else
        self.rtd = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @rtd.nil?
        invalid_properties.push('invalid value for "rtd", rtd cannot be nil.')
      end

      if @rtd < 0
        invalid_properties.push('invalid value for "rtd", must be greater than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if @rtd.nil?
      return false if @rtd < 0
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] rtd Value to be assigned
    def rtd=(rtd)
      if rtd.nil?
        fail ArgumentError, 'rtd cannot be nil'
      end

      if rtd < 0
        fail ArgumentError, 'invalid value for "rtd", must be greater than or equal to 0.'
      end
