...
module HandyAPI
  class OTAApi
    ...
    # Get the latest available firmware available for the provided model and branch.
    # @param model [Model] The model to get latest firmware for.
    # @param branch [Branch] The branch to get latest firmware for.
    # @param [Hash] opts the optional parameters
    # @return [OtaLatestResponse]
    def latest(model, branch, opts = {})
      data, _status_code, _headers = latest_with_http_info(model, branch, opts)
      data
    end
    ...
    # @param branch [Branch] The branch to get latest firmware for.
    # @param [Hash] opts the optional parameters
    # @return [Array<(OtaLatestResponse, Integer, Hash)>] OtaLatestResponse data, response status code and response headers
    def latest_with_http_info(model, branch, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: OTAApi.latest ...'
      end
      # verify the required parameter 'model' is set
      if @api_client.config.client_side_validation && model.nil?
        fail ArgumentError, "Missing the required parameter 'model' when calling OTAApi.latest"
      end
      # verify the required parameter 'branch' is set
      if @api_client.config.client_side_validation && branch.nil?
        fail ArgumentError, "Missing the required parameter 'branch' when calling OTAApi.latest"
      end
      # resource path
      local_var_path = '/ota/latest'
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'OtaLatestResponse'

      # auth_names
      auth_names = opts[:debug_auth_names] || []

      new_options = opts.merge(
        :operation => :"OTAApi.latest",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: OTAApi#latest\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
